 ...
 */
 ...
 /**
 *	@file CorreccionFechaInicioController.php
 *
 *	@author Jose Rafael Perez Teran
 *
 *	@date 17-08-2016
 *
 *  @class CorreccionFechaInicioController
 *	@brief Clase CorreccionFechaInicioController del lado del contribuyente backend.
 *
 *
 *	@property
 *
 ...
 */
 	...
 	namespace backend\controllers\aaee\correccionfechainicio;
	...
	/**
	 * Clase principal que controla la creacion de solicitudes de Correccion de Fecha Inicio de
	 ...
	 */
	class CorreccionFechaInicioController extends Controller
	{
		 ...
		 * de correccion de fecha inicio de actividad. Tambien gestiona la ejecucion
		 * de las reglas de validacion del formulario.
		 * @return view
		 */
		      		...
		      		if ( $model->validate() ) {
	      				...
	      				if ( isset($postData['btn-create']) ) {
	      					if ( $postData['btn-create'] == 1 ) {
	      						...
	      						return $this->render('@frontend/views/aaee/correccion-fecha-inicio/pre-view-create', [
	      																	'model' => $model,
		  		...
		  		if ( count($datos) > 0 ) {
		  			...
		  			if ( count($datoSedePrincipal) > 0 ) {
						...
						$subCaption = Yii::t('frontend', 'Info of Taxpayer');
		  				return $this->render('@frontend/views/aaee/correccion-fecha-inicio/_create', [
					  											'model' => $model,
					  											'datos' => $datos,
		 ...
		 * @param model $model modelo de CorreccionFechaInicioForm.
			...
			if ( isset($_SESSION['idContribuyente']) ) {
				if ( isset($_SESSION['conf']) ) {
					...
					if ( $nroSolicitud > 0 ) {
						...
						$result = self::actionCreateCorreccionFechaInicio($this->_conexion,
																	      $this->_conn,
																	      $model,
																	      $conf);
						if ( $result ) {
							$result = self::actionUpdateFechaInicio($this->_conexion,
																	$this->_conn,
																	$model,
														   		    $conf);
							if ( $result ) {
		 ...
		 */
		private static function actionCreateCorreccionFechaInicio($conexionLocal, $connLocal, $model, $conf)
		{
			$result = false;
			$estatus = 0;
			//$user = isset($model->usuario) ? $model->usuario : null;
		...
		/**
		 * Metodo que ejecuta la actualizacion de la fecha inico de actividad comercial del
		 * contribuyente, aplica solo en aquellos casos donde la aprobacion de la solicitud
		 * sea directa.
		 * @param  ConexionController $conexionLocal instancia de la lcase ConexionController.
		 * @param  connection $connLocal instancia de connection
		 * @param  model $model modelo de CorreccionFechaInicioForm.
		 ...
		 */
		private static function actionUpdateFechaInicio($conexionLocal, $connLocal, $model, $conf)
		{
			$result = false;
			if ( $conf['nivel_aprobacion'] == 1 ) {
				$arregloCondicion = ['id_contribuyente' => $model->id_contribuyente];
				$arregloDatos = ['fecha_inicio' => $model->fecha_inicio_new];
		 ...
		 * de datos.
		 * @param  model $model modelo de la instancia CorrecionFechaInicioForm.
    	...
    	private function actionShowSolicitud($findModel, $modelSearch)
    	{
    		if ( isset($findModel) && isset($modelSearch) ) {
				$opciones = [
					'quit' => '/aaee/correccionfechainicio/correccion-fecha-inicio/quit',
				];
				return $this->render('@frontend/views/aaee/correccion-fecha-inicio/_view', [
																'codigo' => 100,
																'model' => $findModel,
