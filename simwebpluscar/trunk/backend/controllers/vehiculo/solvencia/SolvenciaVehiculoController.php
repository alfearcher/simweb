 ...
 */
 	...
 	namespace backend\controllers\vehiculo\solvencia;
	...
	class SolvenciaVehiculoController extends Controller
	{
		...
		public function actionCheck()
		{
			...
			if ( isset($_SESSION['idContribuyente']) ) {
				...
				if ( count($mensajes) == 0 ) {
					...
					return $this->render('@frontend/views/vehiculo/solvencia/mensaje-error',[
														'mensajes' => $mensajes
							]);
				}
				...
				if ( isset($postData['btn-back-form']) ) {
					...
					if ( $postData['btn-create'] == 5 ) {
						if ( $model->load($postData) ) {
			    			if ( $model->validate() && trim($controlSeleccion) == '' ) {
			    				...
			    				return $this->render('@frontend/views/vehiculo/solvencia/pre-view-create-solvencia',[
		  															'model' => $model,
		  															'caption' => $caption,
		  		...
		  		if ( count($findModel) > 0 ) {
		  			...
		  			if ( $provider !== null ) {
			  			...
			  			return $this->render('@frontend/views/vehiculo/solvencia/_list',[
			  												'model' => $model,
			  												'caption' => $caption,
			  												'subCaption' => $subCaption,
			  												'dataProvider' => $provider,
    	...
    	public function actionMostrarSolicitudCreada($models)
    	{
    		if ( count($models) > 0 ) {
	    		...
	    		$opciones = [
					'quit' => '@frontend/views/vehiculo/solvencia/solvencia-vehiculo/quit',
				];

				$caption = Yii::t('frontend', 'Solicitud Creada');
				$subCaption = Yii::t('frontend', 'Solicitud');
	    		return $this->render('@frontend/views/vehiculo/solvencia/_view',[
												'model' => $models,
    	...
    	private function actionShowSolicitud($findModel, $modelSearch, $dataProvider)
    	{
    		if ( isset($findModel) && isset($modelSearch) ) {
				...
				return $this->render('@frontend/views/aaee/solvencia/_view', [
														'codigo' => 100,
														'model' => $model,
