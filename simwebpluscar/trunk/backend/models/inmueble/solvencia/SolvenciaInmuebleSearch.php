 ...
 */
 ...
 /**
 *  @file SolvenciaInmuebleSearch.php
 *
 *  @author Jose Rafael Perez Teran
 *
 *  @date 25-11-2016
 *
 *  @class SolvenciaInmuebleSearch
 *  @brief Clase Modelo
 *
 *
 *  @property
 ...
 */
	...
	*/
	class SolvenciaInmuebleSearch extends SolvenciaInmueble
	{
		...
		{
			$findModel = self::findSolicitudSolvenciaInmuebleModel();
		...
		/**
		 * Metodo que permite obtener el modelo general de consulta de los inmuebles
		 * @return InmuebleConsulta
		 */
		private function findInmuebleGeneralModel()
		{
		...
		/**
		 * Metodo que retorna el modelo de consulta de los inmuebles activos.
		 * En la consulta se obtienen los atributos que se encuentran en el
		 * select.
		 * @return InmuebleConsulta.
		 */
		public function findInmuebleActivoContribuyente($chkIdImpuesto = [])
		{
			$model = null;

			if ( $this->_id_contribuyente > 0 ) {
				// Se obtiene modelo general de consulta.
				$findModel = self::findInmuebleGeneralModel();
				if ( count($chkIdImpuesto) == 0 ) {
					...
					$model = $findModel->select([
										...
										->andWhere('inactivo =:inactivo',
														[':inactivo' => 0])
										->andWhere(['IN', 'id_impuesto', $chkIdImpuesto])
										->orderBy([
											'direccion' => SORT_ASC,
										])
										->asArray()
		...
		/**
		 * Metodo que crea un provider de datos, sobre los inmuebles existentes
		...
		public function getDataProviderInmueble($chkIdImpuesto = [])
		{
			...
			// Lista de los inmuebles activos que presenta el contribuyente.
			$inmuebles = self::findInmuebleActivoContribuyente($chkIdImpuesto);

			if ( count($inmuebles) > 0 ) {
				foreach ( $inmuebles as $inmueble ) {
					$condicion = '';
					$this->_id_impuesto = $inmueble['id_impuesto'];
					$condicion = [];
					$bloquear = 0;

					// Aqui se debe colocar las condiciones que un objeto no debe presentar.
					$condicion = self::validarCondicionInmueble(date('Y'));
					...
					$data[$inmueble['id_impuesto']] = [
						'id_impuesto' => $inmueble['id_impuesto'],
						'id_contribuyente' => $inmueble['id_contribuyente'],
						'descripcion' => $inmueble['direccion'],

						'catastro' => 0,	// Aqui se llama a la clase o metodo para obtener el nro catastral.

						'inactivo' => $inmueble['inactivo'],
		 ...
		 * @return active record
		 */
		public function findSolicitudSolvenciaInmuebleModel()
		{
			return SolvenciaInmueble::find()->alias('S')
		...
		/***/
		public function validarCondicionInmueble($añoImpositivo)
		{
			$mensaje = [];
