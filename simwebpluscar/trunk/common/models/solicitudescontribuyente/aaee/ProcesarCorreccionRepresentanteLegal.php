 ...
 */
     ...
     */
    class ProcesarCorreccionRepresentanteLegal extends CorreccionRepresentanteLegalSearch
    {
        /**
         ...
         * en caso contrario.
         */
        public function findCorreccionRepresentante()
        {
            // Este find retorna el modelo de la entidad "sl-correcciones-representante-legal".
            $findModel = $this->findSolicitudCorreccionRepresentanteLegal($this->_model->nro_solicitud);
        ...
        {
            $result = false;
            // modelo de CorreccionRepresentanteLegal. Uno o varios registros.
            $modelCorreccion = self::findCorreccionRepresentante();
            if ( $modelCorreccion !== null ) {
                // Entidad "sl-".
                $result = self::updateSolicitudCorreccionRepresentanteLegal($modelCorreccion);
                if ( $result ) {
                    $result = self::updateRepresentante($modelCorreccion);
                }
            } else {
         ...
         * sobre la solicitud.
         * @param  Active Record $modelCorreccion modelo de la entidad "sl-correciones-representante-legal"
         * (CorreccionRepresentanteLegal). Este modelo contiene los datos-detalles, referida a los
         * datos cargados al momento de elaborar la solicitud.
         * @return boolean retorna un true si todo se ejecuto satisfactoriamente, false
         * en caso contrario.
         */
        private function updateSolicitudCorreccionRepresentanteLegal($modelCorreccion)
        {
            $result = false;
         ...
         * @param  model $modelCorreccion modelo de CorreccionRepresentanteLegal.
         * Contiene llos datos del find.
         * @return boolean retorna true si ejecuta la actualizacion de los DNI, false
         * en caso contrario.
         */
        private function updateRepresentante($modelCorreccion)
        {
            $result = false;
            ...
            foreach ( $modelCorreccion as $key => $value ) {
                ...
                $arregloDatos = [
                    'naturaleza_rep' => $modelCorreccion[$key]->naturaleza_rep_new,
                    'cedula_rep' => $modelCorreccion[$key]->cedula_rep_new,
                    'representante' => $modelCorreccion[$key]->representante_new,
                ];
