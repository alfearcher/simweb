...
use common\models\propaganda\patrocinador\SlPropagandasPatrocinadores;
     ...
     */
    class ProcesarAsignacionPatrocinadorPropaganda extends SlPropagandas
    {
         ...
         * sobre la entidad "sl-propagandas-patrocinadores", referente al detalle de la solicitud. Es la
         * entidad donde se guardan los detalle de esta solicitud.
         * @return Boolean Retorna un true si todo se ejecuto satisfactoriamente, false
         * en caso contrario.
         */
        public function findAsignacionPatrocinadorPropaganda()
        {
            // Este find retorna el modelo de la entidad "sl-propagandas-patrocinadores"
            // con datos, ya que en el metodo padre se ejecuta el ->one() que realiza
            // la consulta.
            $SlPropagandasPatrocinadores = New SlPropagandasForm($this->_model->id_contribuyente);

            $modelFind = $SlPropagandasPatrocinadores->findPatrocinadorPropaganda($this->_model->nro_solicitud);
        ...
        */
        ...
        private function aprobarDetalleSolicitud()
        {
           // die('llego a aprobar');
            $result = false;
            $modelPatrocinador = self::findAsignacionPatrocinadorPropaganda();
            //die(var_dump($modelInscripcion));
            if ( $modelPatrocinador !== null ) {
                if ( $modelPatrocinador['id_contribuyente'] == $this->_model->id_contribuyente ) {
                    //die('comparo');
                    $result = self::updateSolicitudAsignacionPatrocinador($modelPatrocinador);
                    // if ( $result ) {
         ...
         * en caso contrario.
         */
        private function updateSolicitudAsignacionPatrocinador($modelPatrocinador)
        { 
           // die('llego a solicitud inscripcion');
            ...
            $model = New SlPropagandasPatrocinadores();
            ...
            $camposModel = $modelPatrocinador->toArray();
            ...
            if ( !$cancel ) {
                if($arregloDatos['estatus'] == 9)
                {
                    ...
                    $arregloDatosMaster = [
                                            ...
                                            'id_patrocinador' => $camposModel['id_patrocinador'],
                                            'estatus' => 0,
