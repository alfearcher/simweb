 ...
 */
 ...
 /**
 *  @file ProcesarAnulacionPatrocinadorPropaganda.php
 *
 *  @author Manuel Alejandro Zapata Canelon
 *
 *  @date 15/09/2016
 *
 *  @class ProcesarAnulacionPatrocinadorPropaganda
 *  @brief
 *
 *
 *
 ...
 */
     ...
     */
    class ProcesarAnulacionPatrocinadorPropaganda extends SlPropagandas
    {
         ...
         * sobre la entidad "sl-anulaciones-patrocinadores", referente al detalle de la solicitud. Es la
         * entidad donde se guardan los detalle de esta solicitud.
         * @return Boolean Retorna un true si todo se ejecuto satisfactoriamente, false
         * en caso contrario.
         */
        public function findAnulacionPatrocinadorPropaganda()
        {
            // Este find retorna el modelo de la entidad "sl-anulaciones-patrocinadores"
            // con datos, ya que en el metodo padre se ejecuta el ->one() que realiza
            // la consulta.
            $SlAnulacionesPatrocinadores = New SlPropagandasForm($this->_model->id_contribuyente);

            $modelFind = $SlAnulacionesPatrocinadores->findAnularPatrocinadorPropaganda($this->_model->nro_solicitud);
        ...
        */
        ...
        private function aprobarDetalleSolicitud()
        {
           // die('llego a aprobar');
            $result = false;
            $modelAnulacion = self::findAnulacionPatrocinadorPropaganda();
            //die(var_dump($modelInscripcion));
            if ( $modelAnulacion !== null ) {
                if ( $modelAnulacion['id_contribuyente'] == $this->_model->id_contribuyente ) {
                    //die('comparo');
                    $result = self::updateSolicitudAnulacionPatrocinador($modelAnulacion);
                    // if ( $result ) {
         ...
         * en caso contrario.
         */
        private function updateSolicitudAnulacionPatrocinador($modelAnulacion)
        { 
           // die('llego a solicitud inscripcion');
            ...
            $model = New SlAnulacionesPatrocinadores();
            ...
            $camposModel = $modelAnulacion->toArray();
           //die(var_dump($camposModel));
            ...
            if ( !$cancel ) {
                if($arregloDatos['estatus'] == 9)
                {
                    ...
                    $tableNameMaster = 'propagandas_patrocinadores';
                ...
                $arregloCondicionMaster = [
                                        ...
                                        //  die(var_dump($arregloCondicionMaster));
