 ...
 */
    ...
    use frontend\models\vehiculo\solicitudes\SlReposicionCalcomania;
     ...
     */
    class ProcesarCambioCalcomaniaExtravio extends SlReposicionCalcomania
    {
         ...
         * en caso contrario.
         */
        public function findSolicitudCalcomaniaExtravio()
        {
           // die('llegue al find cambio propietario');
            ...
            // la consulta.
            $SlCalcomania = New SlVehiculosForm($this->_model->id_contribuyente);

            $modelFind = $SlCalcomania->findSolicitudReposicionCalcomaniaExtravio($this->_model->nro_solicitud);
        ...
        */
        ...
        private function aprobarDetalleSolicitud()
        {
           // die('llego a aprobar');
            $result = false;
            $modelCalcomania = self::findSolicitudCalcomaniaExtravio();
            //die(var_dump($modelInscripcion));
            if ( $modelCalcomania !== null ) {
                if ( $modelCalcomania['id_contribuyente'] == $this->_model->id_contribuyente ) {
                    //die('comparo');
                    $result = self::updateSolicitudCambioCalcomaniaExtravio($modelCalcomania);
                    // if ( $result ) {
        ...
        {
            $result = false;
            $modelCambioCalcomania = self::findSolicitudCalcomaniaExtravio();
            if ( $modelCambioCalcomania !== null ) {
                if ( $modelCambioCalcomania['id_contribuyente'] == $this->_model->id_contribuyente ) {
                    $result = self::updateSolicitudCambioCalcomaniaExtravio($modelCambioCalcomania);
                } else {
                    self::setErrors(Yii::t('backend', 'Error in the ID of taxpayer'));
                }
            }
         ...
         * en caso contrario.
         */
        private function updateSolicitudCambioCalcomaniaExtravio($modelCambioCalcomania)
        { 
           // die('llego a solicitud inscripcion');
            ...
            $model = New SlReposicionCalcomania();
            ...
            $camposModel = $modelCambioCalcomania->toArray();
            ...
            if ( !$cancel ) {
                ...
                $arregloCondicionMaster = [
                                            ...
                                            'id_calcomania' => $camposModel['id_impuesto'],
