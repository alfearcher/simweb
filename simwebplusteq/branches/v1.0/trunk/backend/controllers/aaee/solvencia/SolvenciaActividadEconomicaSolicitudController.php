		...
		public function actionCheck()
		{
			...
			if ( isset($_SESSION['idContribuyente']) ) {
				...
				if ( count($mensajes) == 0 ) {
					...
					return $this->render('@frontend/views/aaee/solvencia/mensaje-error',[
														'mensajes' => $mensajes
							]);
				}
			...
			if ( isset($_SESSION['idContribuyente']) && isset($_SESSION['begin']) && isset($_SESSION['conf']) ) {
				...
				if ( isset($postData['btn-back-form']) ) {
					...
					if ( $postData['btn-create'] == 5 ) {
						if ( $model->load($postData) ) {
			    			if ( $model->validate() ) {
			    				...
			    				return $this->render('@frontend/views/aaee/solvencia/pre-view-create',[
		  															'model' => $model,
		  															'caption' => $caption,
		  								]);
							}
		  		...
		  		if ( count($findModel) > 0 ) {
		  			...
		  			return $this->render('@frontend/views/aaee/solvencia/_create',[
		  												'model' => $model,
		  												'caption' => $caption,
		  					]);
