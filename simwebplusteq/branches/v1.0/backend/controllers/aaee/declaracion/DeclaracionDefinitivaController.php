 ...
 *	@brief Clase DeclaracionDefinitivaController del lado del contribuyente backend.
 *
 *
 *	@property
 *
 ...
 */
 	...
 	namespace backend\controllers\aaee\declaracion;
	...
	class DeclaracionDefinitivaController extends Controller
	{
			...
			if ( isset($_SESSION['idContribuyente']) && isset($_SESSION['begin']) && isset($_SESSION['conf'])) {
				...
				if ( isset($postData['btn-back-form']) ) {
		  		...
		  		if ( isset($findModel) ) {
					...
					return $this->render('@frontend/views/aaee/declaracion/definitiva/_create',
																[
																	'model' => $model,
																	'caption' => $caption,
				...
				if ( count($lapso) > 0 ) {
					...
					if ( isset($postData['btn-back-form']) ) {
						if ( $postData['btn-back-form'] == 1 ) {
							...
							return $this->render('@frontend/views/aaee/declaracion/definitiva/declaracion-definitiva-form', [
						...
						if ( $postData['btn-create'] == 3 ) {
							...
							if ( $suma <= 0 ) {
								$mensajeDeclaracion = Yii::t('frontend', 'LA SUMA DEL MONTO DECLARADO NO CUMPLE CON LO REQUERIDO. DEBE SER MAYOR A LA SUMA DE LAS ESTIMADAS CERO (0).');
								$result = false;
							}
							...
							if ( $result ) {
								...
								return $this->render('@frontend/views/aaee/declaracion/definitiva/pre-view-create', [
																	'model' => $modelMultiplex,
																	'findModel' => $findModel,
																	'caption' => $caption,
	  																'opciones' =>$opciones,
    	...
    	private function actionShowSolicitud($findModel, $modelSearch, $dataProvider)
    	{
    		if ( isset($findModel) && isset($modelSearch) ) {
				...
				return $this->render('@frontend/views/aaee/declaracion/definitiva/_view', [
																'codigo' => 100,
																'model' => $model,
