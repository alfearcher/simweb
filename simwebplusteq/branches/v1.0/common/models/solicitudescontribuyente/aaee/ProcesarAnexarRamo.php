 ...
 */
 ...
 /**
 *  @file ProcesarAnexarRamo.php
 *
 *  @author Jose Rafael Perez Teran
 *
 *  @date 01/09/2016
 *
 *  @class ProcesarAnexarRamo
 *  @brief
 *
 *
 *
 ...
 */
     ...
     */
    class ProcesarAnexarRamo extends AnexoRamoSearch
    {
        /**
         ...
         * en caso contrario.
         */
        public function findAnexarRamo()
        {
            // Este find retorna el modelo de la entidad "sl-anexos-ramos".
            $findModel = $this->findSolicitudAnexoRamo($this->_model->nro_solicitud);
        ...
        private function aprobarDetalleSolicitud()
        {
            $result = false;
            $idGenerado = 0;
            // modelo de AnexoRamo. Uno o varios registros.
            $modelRamo = self::findAnexarRamo();
            if ( $modelRamo !== null ) {
                // Entidad "sl-".
                $result = self::updateSolicitudAnexarRamo($modelRamo);
                if ( $result ) {
                    $result = self::createActEconIngresos($modelRamo);
                }
            } else {
         ...
         * sobre la solicitud.
         * @param  Active Record $modelRamo modelo de la entidad "sl-anexos-ramos"
         * (AnexoRamo). Este modelo contiene los datos-detalles, referida a los
         * datos cargados al momento de elaborar la solicitud.
         * @return boolean retorna un true si todo se ejecuto satisfactoriamente, false
         * en caso contrario.
         */
        private function updateSolicitudAnexarRamo($modelRamo)
        {
            $result = false;
        ...
        /**
         * Metodo que guarda el detalle de los ramos anexados, por cada ramo
         * se realiza una insercion en la entidad que guarda los ramos.
         * @param active record $modelRamo modelo de AnexoRamo, entidad "sl-anexos-ramos".
         * @return boolean retorna true si guarda, false en caso contrario.
         */
        private function createActEconIngresos($modelRamo)
        {
            $result = false;
            if ( $modelRamo !== null ) {

                // Se determina el identificador de la entidad principal que guarda el registro
                // maestro de la declaracion (act-econ). Para utilizarlo como link de los detalles
                // de los ramos que se anexaran a dicha declaracion. Estos ramos iran en la entidad
                // detalle (act-econ-ingresos).
                $searchRamo = New AnexoRamoSearch($modelRamo[0]->id_contribuyente);
                $idImpuesto = $searchRamo->getIdentificadorLapsoValido($modelRamo[0]->ano_impositivo);
