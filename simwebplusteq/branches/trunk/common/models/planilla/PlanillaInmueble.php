 ...
 */
 ...
 /**
 *  @file PLanillaInmueble.php
 *
 *  @author Jose Rafael Perez Teran
 *
 *  @date 17-04-2016
 *
 *  @class PLanillaInmueble
 *  @brief Clase Modelo
 *
 *
 *  @property
 ...
 */
	...
	* 	Clase
	*/
	class PlanillaInmueble extends Planilla
	{
		...
		const IMPUESTO = 2;			// Impuesto asociado a vehiculo.
		...
		/***/
		public function liquidarEstimadaInmueble()
		{
			if ( isset($this->conexion) && isset($this->conn) && $this->_idImpuesto > 0 ) {
				...
				$this->configurarLapsoLiquidacionInmueble();
				...
				if ( $ciclo != null ) {
					...
					if ( $result != null ) {
						$this->_idContribuyente = $this->_datosInmueble['id_contribuyente'];
				...
				foreach ( $cicloLiquidacion as $key => $value ) {
					...
					if ( strlen($key) == 4 && is_integer($key) ) {
						if ( is_array($value) ) {
							...
							$montoCalculado = $this->LiquidarDeclaracionInmueble($año);
		...
		private function configurarLapsoLiquidacionVehiculo()
		{
			...
			if ( $ultimo == null ) {
				...
				$this->_añoInicio = $this->_datosInmueble['ano_inicio'];
				if ( $this->_añoInicio > 0 ) {
					$año = $this->_añoInicio;
					$año = (int) $año;
					...
					if ( strlen(trim($this->_añoDesde)) == 4 ) {
						...
						// y el año. Para este caso el impuesto es Inmueble y el año estara
						// definido por la año inicio ($this->_añoInicio). Lo que se debe determinar
						...
						$this->_periodoDesde = OrdenanzaBase::getPeriodoSegunFecha($exigibilidadLiq['exigibilidad'], $this->_añoInicio);
		 ...
		 * del inmueble, se obtiene entre otros datos la planilla y los detalles de la misma.
		 * @return Array, Retorna un array con los datos principales de la entidad "pagos" y la entidad
		 * "pagos-detalle".
		 */
		public function getUltimoLapsoInmueble()
		{
		 ...
		 */
		public function LiquidarDeclaracionInmueble($año)
		{
			$monto = 0;		// Calculo anual de la liquidacion.
			if ( $this->_idImpuesto > 0 ) {
				$liquidacion = New LiquidacionInmueble($this->_idImpuesto);
				$liquidacion->setAnoImpositivo($año);
				$liquidacion->iniciarCalcularLiquidacionInmueble();
