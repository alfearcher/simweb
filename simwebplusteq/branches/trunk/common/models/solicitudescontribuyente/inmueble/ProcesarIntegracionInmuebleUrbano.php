 ...
 */
 ...
 /**
 *  @file ProcesarIntegracionInmuebleUrbano.php
 *
 *  @author Alvaro Jose Fernandez ARcher
 *
 *  @date 08/08/2016
 *
 *  @class ProcesarIntegracionInmuebleUrbano
 *  @brief
 *
 *
 *
 ...
 */
     ...
     */
    class ProcesarIntegracionInmuebleUrbano extends SlInmueblesUrbanosSearch
    {
        private $_model;
        ...
        public function findActualizacionInmuebleUrbano()
        {
            ...
            // la consulta.
            $modelFind = $this->findIntegracion($this->_model->nro_solicitud);
        ...
        */
        ...
        private function aprobarDetalleSolicitud()
        {
            ...
            if ( $modelInscripcion !== null ) {
                if ( $modelInscripcion[0]['id_contribuyente'] == $this->_model->id_contribuyente ) {
                    $result = self::updateSolicitudIntegracion($modelInscripcion);
                    // if ( $result ) {
         ...
         * en caso contrario.
         */
        private function updateSolicitudIntegracion($modelInscripcion)
        { 
            $result = false;
            ...
            if ( !$cancel ) {
                if($arregloDatos['estatus_funcionario'] == 9)
                {
                    ...
                    $arregloCondicionMasterInactivacion = [
                                                'id_impuesto' => $camposModel[1]['id_impuesto'],
                                              ]; 
                    $arregloCondicionMasterInactivacion2 = [
                                                'id_impuesto' => $camposModel[2]['id_impuesto'],
                                              ];

                    
                    $arrayDatosMaster = [   'direccion' => $camposModel[0]['direccion'],
                                            'casa_edf_qta_dom' => $camposModel[0]['casa_edf_qta_dom'], 
                                            'piso_nivel_no_dom' => $camposModel[0]['piso_nivel_no_dom'], 
                                            'apto_dom' => $camposModel[0]['apto_dom'], 
                                            'medidor' => $camposModel[0]['medidor'], 
                                            'observacion' => $camposModel[0]['observacion'],
                                            'tipo_ejido' => $camposModel[0]['tipo_ejido'],
                                      ]; 


                    $resultUpdate = $this->_conexion->modificarRegistro($this->_conn, $tableNameMaster, $arregloDatosMasterInactivacion, $arregloCondicionMasterInactivacion);
                    $resultUpdate2 = $this->_conexion->modificarRegistro($this->_conn, $tableNameMaster, $arregloDatosMasterInactivacion, $arregloCondicionMasterInactivacion2);
                    $resultInsert = $this->_conexion->guardarRegistro($this->_conn, $tableNameMaster, $arrayDatosMaster);
