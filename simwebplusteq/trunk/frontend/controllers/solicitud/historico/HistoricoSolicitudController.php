 ...
 */
 	...
 	namespace frontend\controllers\solicitud\historico;
	 ...
	 * Opcion del lado del contribuyente (frontend).
	 */
	class HistoricoSolicitudController extends Controller
	{
		...
		public function actionMostrarFormConsultaSolicitud()
		{
			...
			if ( isset($_SESSION['idContribuyente']) && trim($usuario) !== '' ) {
				...
				$esFuncionario = false;
				$tipoNaturaleza = $model->getTipoNatutaleza($idContribuyente);
				...
				// url donde se solicitara exportar el reporte.
		      	...
		      	if ( $model->load($postData) ) {
		      		if ( $model->validate() ) {
						$subCaption = Yii::t('backend', 'Resultado de la Consulta');
		      			$dataProvider = $model->getDataProviderFrontend($idContribuyente);
		      			return $this->render('@backend/views/reporte/solicitud/historico/historico-solicitud-reporte', [
		      	...
		      	if ( $tipoNaturaleza == 'JURIDICO' ) {
		      		...
		      		$listaImpuesto = $model->getListaImpuestoContribuyenteNatural();
		      	}
		      	...
		      	//$urlRequest = 'reporte/solicitud/historico/historico-solicitud-reporte';
		      	...
		      	return $this->render('@backend/views/reporte/solicitud/historico/historico-solicitud-consulta-form', [
				      											'model' => $model,
				      											'caption' => $caption,
				      											'subCaption' => $subCaption,
		    	...
		    	// No esta deinido el contribuyente.
