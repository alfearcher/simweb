 ...
 *	@brief ClaseSolvenciaInmuebleController del lado del contribuyente frontend.
 *
 *
 *	@property
 *
		...
		public function actionCheck()
		{
			...
			if ( isset($_SESSION['idContribuyente']) ) {
				...
				if ( count($mensajes) == 0 ) {
					...
					return $this->render('/inmueble/solvencia/mensaje-error',[
														'mensajes' => $mensajes
							]);
				}
				...
				if ( isset($postData['btn-back-form']) ) {
					...
					if ( $postData['btn-create'] == 5 ) {
						if ( $model->load($postData) ) {
			    			if ( $model->validate() && trim($controlSeleccion) == '' ) {
			    				...
			    				return $this->render('/inmueble/solvencia/pre-view-create-solvencia',[
		  															'model' => $model,
		  															'caption' => $caption,
		  		...
		  		if ( count($findModel) > 0 ) {
		  			...
		  			if ( $provider !== null ) {
			  			...
			  			return $this->render('/inmueble/solvencia/_list',[
			  												'model' => $model,
			  												'caption' => $caption,
			  												'subCaption' => $subCaption,
			  												'dataProvider' => $provider,
