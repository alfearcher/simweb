 ...
 *	@brief Clase CorreccionRazonSocialController del lado del contribuyente frontend.
 *
 *
 *	@property
 *
 ...
 */
	...
	/**
	 * Clase principal que controla la creacion de solicitudes de Correccion de la Razon Social.
	...
	class CorreccionRazonSocialController extends Controller
	{
		 ...
		 * de correccion de la razon social. Tambien gestiona la ejecucion de las reglas
			...
			if ( isset($_SESSION['idContribuyente']) && isset($_SESSION['begin']) && isset($_SESSION['conf'])) {
				...
				if ( isset($postData['btn-quit']) ) {
	      				...
	      				if ( isset($postData['btn-create']) ) {
	      					if ( $postData['btn-create'] == 1 ) {
	      						...
	      						if ( count($ids) > 0 ) {
		      						...
		      						return $this->render('@frontend/views/aaee/correccion-razon-social/pre-view-create', [
		      																	'model' => $model,
		  		...
		  		if ( isset($datos) ) {
		  			...
		  			$subCaption = Yii::t('frontend', 'Info of Taxpayer');
		  			return $this->render('@frontend/views/aaee/correccion-razon-social/_create', [
					  											'model' => $model,
					  											'datos' => $datos,
					  											'subCaption' => $subCaption,
					  											'dataProvider' => $dataProvider,
			...
			if ( isset($_SESSION['idContribuyente']) ) {
				if ( isset($_SESSION['conf']) ) {
					...
					if ( $nroSolicitud > 0 ) {
						...
						$result = self::actionCreateCorreccionRazonSocial($this->_conexion,
																	      $this->_conn,
																	      $model,
																	      $conf,
																	      $chkSeleccion);
						...
						if ( $result ) {
							$result = self::actionUpdateRazonSocial($this->_conexion,
															        $this->_conn,
																    $model,
																    $conf,
																    $chkSeleccion);
		 ...
		 */
		private static function actionCreateCorreccionRazonSocial($conexionLocal, $connLocal, $model, $conf, $chkSeleccion)
		{
			$result = false;
			$estatus = 0;
			//$user = isset($model->usuario) ? $model->usuario : null;
		...
		/**
		 * Metodo que ejecuta la actualizacion de la razon social del conjuto de contribuyente,
		...
		public function actionView($id)
    	{
    		if ( isset($_SESSION['idContribuyente']) ) {
	    		if ( $id > 0 ) {
	    			$searchCorreccion = New CorreccionRazonSocialSearch($_SESSION['idContribuyente']);
	    			$findModel = $searchCorreccion->findSolicitudCorreccionRazonSocial($id);
    	...
    	private function actionShowSolicitud($findModel, $modelSearch, $dataProvider)
    	{
    		if ( isset($findModel) && isset($modelSearch) ) {
				...
				$opciones = [
					'quit' => '/aaee/correccionrazonsocial/correccion-razon-social/quit',
				];
				return $this->render('@frontend/views/aaee/correccion-razon-social/_view', [
																'codigo' => 100,
																'model' => $model,
