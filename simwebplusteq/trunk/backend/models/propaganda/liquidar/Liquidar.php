 ...
 */
	...
	*/
	class Liquidar
	{
		...
		private $_liquidarPropaganda;		// Instancia de la clase LiquidacionPropaganda().
		private $_tipoLiquidacion;
		private $_objeto;				// Instancia de la clase Propaganda().
		...
		public function __construct($idContribuyente, $idImpuesto)
		{
			...
			$this->_objeto = Propaganda::findOne($idImpuesto);
			$this->_liquidarPropaganda = New LiquidacionPropaganda($idImpuesto);
			$this->_id_pago = 0;
		}
				...
				if ( count($rangoInicio) > 0 ) {
					...
					for ( $i = (int)$añoInicio; $i <= (int)$añoFinal; $i++ ) {
						if ( (int)$añoInicio == (int)$añoFinal ) {
							...
							self::liquidarAnoImpositivoPropaganda($i, $periodoInicio, $periodoFinal);
		 ...
		 */
		private function liquidarAnoImpositivoPropaganda($año, $desdePeriodo, $hastaPeriodo = 0)
		{
			...
			if ( count($exigibilidadLiq) > 0 ) {
				...
				$this->_liquidarPropaganda->setAnoImpositivo($año);
				$montoCalculado = $this->_liquidarPropaganda->iniciarCalcularLiquidacionPropaganda();

				if ( $montoCalculado > 0 ) {
					$exigLiq = (int)$exigibilidadLiq['exigibilidad'];
					$montoPeriodo = $montoCalculado;
					...
					for ( $i = $desdePeriodo; $i <= $hastaPeriodo; $i++) {
						...
						$modelDetalle[$j]->descripcion = 'LIQUIDACION DE PROPAGANDA COMERCIAL';
