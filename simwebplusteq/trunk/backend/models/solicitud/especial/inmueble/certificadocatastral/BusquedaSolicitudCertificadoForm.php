 ...
 */
	...
	use backend\models\inmueble\SlCertificadoCatastralForm;
	...
	*/
	class BusquedaSolicitudCertificadoForm extends Model
	{
	    ...
	    */
	     ...
	     * @return [type] [description]
	     */
		public function findSolicitudCertificadoModel()
		{ 
			return $findModel = SlCertificadoCatastralForm::find()->alias('L');
		}
		 ...
		 * @return [type] [description]
		 */
		public function findSolicitudCertificado()
		{
			$findModel = self::findSolicitudCertificadoModel();
		 ...
		 * @return [type]         [description]
		 */
		public function getDatoSolicitudCertificado($params)
		{
			$findModel = self::findSolicitudCertificado();
			$this->load($params);
		 ...
		 */
		public function getSolicitudCertificadoSeleccionada($chkNroSolicituds)
		{
			$findModel = self::findSolicitudCertificado();
		...
		public function armarDataProvider($params, $preView = false)
		{
			...
			$provider = [];
			if ( !$preView ) {
				$results = self::getDatoSolicitudCertificado($params);
			} else { 
				$results = self::getSolicitudCertificadoSeleccionada($params);
			}
			...
			if ( $results !== null && count($results) > 0 ) {
				...
				foreach ( $results as $result ) {
					...
					$data[$result['nro_solicitud']] = [
						...
						'licencia' => $result['certificado_catastral'],
